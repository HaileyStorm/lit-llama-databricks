...
# Give it the name alpaca to fit it into the existing lit-llama pipeline
...
def prepare(
    ...
    """Prepare the Databricks dataset for instruction tuning.
...
def prepare_sample(example: dict, tokenizer: Tokenizer, max_length: int, mask_inputs: bool = True):
    ...
    the instruction and the context. The label/target is the same message but with the
    response attached.
...
def generate_prompt(example):
    """Generates a standardized message to prompt the model with an instruction, optional context and a
    'response' field."""
    if example["context"]:
        return (
            "Below is an instruction that describes a task, paired with a context that provides further information. "
