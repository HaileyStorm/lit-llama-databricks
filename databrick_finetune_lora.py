...
def main():
    fabric = L.Fabric(accelerator="cuda", devices="auto", precision="bf16-mixed")
    fabric.launch()
    ...
    config = LLaMAConfig.from_name(model_string)
    config.block_size = block_size

    checkpoint = torch.load(f"checkpoints/lit-llama/{model_string}/state_dict.pth")
    ...
    for iter_num in range(max_iters):
        ...
        if (iter_num + 1) % gradient_accumulation_steps == 0:
            ...
            if step_count % eval_interval == 0:
                ...
                fabric.print(f"step {iter_num} of {max_iters} ({round((iter_num / max_iters) * 100, 2)}%): val loss {val_loss:.4f}")
                fabric.barrier()
